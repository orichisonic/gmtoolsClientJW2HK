...
namespace M_SDO
{
    class Operation_SDO
    {
        ...
        public static int iPageSize = 30;
        /// <summary>
        /// 获取服务器地址列表
        /// </summary>
            ...
            else
            {
                for (int i = 0; i < val.GetLength(0); i++)
                {
                    for (int j = 0; j < val.GetLength(1); j++)
                    {
                        ...
                        if (val[i, j].eName == CEnum.TagName.SDO_Mark)
                        {
                            if (int.Parse(val[i, j].oContent.ToString()) == 4)
                                val[i, j].oContent = "经验";
                            else if (int.Parse(val[i, j].oContent.ToString()) == 5)
                                val[i, j].oContent = "魅力";
                            else if (int.Parse(val[i, j].oContent.ToString()) == 6)
                                val[i, j].oContent = "MVP";
                            else if (int.Parse(val[i, j].oContent.ToString()) == 7)
                                val[i, j].oContent = "段位";
        ...
        private static DataTable BuildColumn(CSocketEvent mEvent, CEnum.Message_Body[,] val)
        {
            ...
            for (int i = 0; i < val.GetLength(1); i++)
            {
                if (val[0, i].eName != CEnum.TagName.PageCount)
                {
                    ...
                    mDataTable.Columns.Add((string)config.ReadConfigValue("GLOBAL", val[0, i].eName.ToString().Trim()), typeof(string));
